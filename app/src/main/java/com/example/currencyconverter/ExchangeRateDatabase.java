...
public class ExchangeRateDatabase {
    ...
    public double getExchangeRate(String currency) {
        // Prüfe zuerst, ob ein aktualisierter Wechselkurs vorhanden ist
        Double updatedRate = exchangeRates.get(currency);
        if (updatedRate != null) {
            return updatedRate;
        }

        // Falls nicht, verwende den Standardwert
        ExchangeRate rate = CURRENCIES_MAP.get(currency);
        if (rate != null) {
            return rate.getRateForOneEuro();
        }
        return 0.0; // Oder einen Fehler werfen, wenn die Währung nicht gefunden wird
    }
    ...
    public void setExchangeRate(String currency, double exchangeRate) {
        ...
        if (rate != null) {
            ...
            // Wenn die Währung noch nicht vorhanden ist, füge sie hinzu.
            rate = new ExchangeRate(currency, "Unbekannt", exchangeRate);
